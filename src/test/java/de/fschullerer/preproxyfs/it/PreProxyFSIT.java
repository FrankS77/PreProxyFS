...
class PreProxyFSIT {
    ...
    public static void setup() throws IOException {
        ...
        String pac2 = "function FindProxyForURL(url, host) {if (host == \"" + UtilT.REMOTE_SERVER_ENDPOINT1 + "\"){"
                              + "return \"PROXY localhost:" + proxyReaderPort1 + ";\"}" + "if (host == \"" 
                              + UtilT.REMOTE_SERVER_ENDPOINT2
                              + "\"){ return \"PROXY localhost:" + proxyReaderPort2 + ";\"} return \"DIRECT\";}";
   ...
   // @Test
    ...
    void assertPreFSIT1() throws IOException {
        String proxy1Req = "GET / HTTP/1.1\r\nHost: " + UtilT.REMOTE_SERVER_ENDPOINT1 + "\r\nUser-agent:foo\r\n";
   ...
   // @Test
    ...
    void assertPreFSIT2() throws IOException {
        String proxy2Req = "GET / HTTP/1.1\r\nHost: " + UtilT.REMOTE_SERVER_ENDPOINT2 + "\r\nUser-agent:foo\r\n";
    ...
    void assertPreFSIT3() throws IOException {
        String connectRequest = "CONNECT localhost:" + directConnectionEndpointPort + " HTTP/1.1\r\nHost: localhost:"
                                        + directConnectionEndpointPort + "\r\n";
        String directReq = "GET http://localhost:" + directConnectionEndpointPort + " HTTP/1.1\r\nHost: localhost\r\nUser-agent:foo\r\n";
        ClientSocketThread testingClient = new ClientSocketThread("localhost", PreProxyFS.DEFAULT_LOCAL_BIND_PORT);
        testingClient.start();
