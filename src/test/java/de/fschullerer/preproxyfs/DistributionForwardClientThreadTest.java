...
class DistributionForwardClientThreadTest {
    ...
    @Mock private ForwardServerThread distributeForwardServerThreadMock;

    @Mock private DirectForwardServer directForwardServerMock;
    ...
    void assertInputReadFromSocketEqualsInput(String request) throws IOException {
        ...
        assertThat(request.getBytes())
                .as("Input and output should be the same!")
                .isEqualTo(readRequestFromClient);
    }

    @Test
    @Tag("UnitTest")
    @DisplayName(
            "DS002: Assert that basic authentication is added if proxy is in proxyAuthenticationMap.")
    void assertBasicAuthIsAdded() {
        ...
        byte[] modifiedRequest =
                ds.addHttpBasicAuthentication(
                        UtilT.STANDARD_REQUEST.getBytes(), UtilT.PROXY1_CORPORATE);
        assertThat(UtilT.STANDARD_REQUEST_WITH_AUTH)
                .as("Proxy-Authorization line should be added to request")
    ...
    @Tag("UnitTest")
    @DisplayName(
            "DS003: Assert that basic authentication is not added if proxy is not in proxyAuthenticationMap.")
    void assertBasicAuthIsNotAdded() {
        ...
        byte[] notModifiedRequest =
                ds.addHttpBasicAuthentication(
                        UtilT.STANDARD_REQUEST.getBytes(), UtilT.PROXY2_CORPORATE);
        assertThat(UtilT.STANDARD_REQUEST)
                .as("Proxy-Authorization line should not be added to request")
    ...
    @Tag("UnitTest")
    @DisplayName(
            "DS004: Assert that basic authentication is not added if there is a proxy authentication in request.")
    void assertBasicAuthIsNotAdded2() {
        ...
        byte[] notModifiedRequest =
                ds.addHttpBasicAuthentication(
                        UtilT.STANDARD_REQUEST_WITH_AUTH.getBytes(), UtilT.PROXY1_CORPORATE);
        assertThat(UtilT.STANDARD_REQUEST_WITH_AUTH)
                .as("Original request should not be modified.")
                .isEqualTo(new String(notModifiedRequest, StandardCharsets.US_ASCII));
    }
    ...
    void assertCorrectProxy() throws ProxyEvaluationException {
        ...
        assertThat(UtilT.PROXY2_CORPORATE)
                .as("Proxy should be: " + UtilT.PROXY2_CORPORATE)
    ...
    void assertDirectConnection() throws ProxyEvaluationException {
        ...
        PreProxyFS.setPacScriptParser(new JavaxPacScriptParser(pacScript));
        String proxyToTake =
                ds.getCorrectProxyForHttpRequest(UtilT.STANDARD_REQUEST_DIRECT_PAC_SCRIPT_V2);
        assertThat(Util.DIRECT).as("DIRECT connection should be used.").isEqualTo(proxyToTake);
    }
    ...
    void testHttpRequestWithStartForwardThread() throws ProxyEvaluationException, IOException {
        ...
        DistributeForwardClientThread ds = new DistributeForwardClientThread(sock);
        doNothing()
                .when(distributeForwardServerThreadMock)
                .setServerSocket(isA(String.class), isA(Integer.class));
        ...
        byte[] modifiedRequest =
                ds.startForwardServerThreadForHttpRequest(UtilT.STANDARD_REQUEST.getBytes());
        assertThat(UtilT.STANDARD_REQUEST_WITH_AUTH)
                .as("Proxy-Authorization line should be added to request")
                .isEqualTo(new String(modifiedRequest, StandardCharsets.US_ASCII));
    }
    ...
    @Tag("UnitTest")
    @DisplayName(
            "DS007: Assert that basic authentication is added for Http request "
    ...
    void testNonHttpRequestWithStartForwardThread() throws ProxyEvaluationException, IOException {
        ...
        byte[] nonModifiedRequest =
                ds.startForwardServerThreadForHttpRequest(originalRequest.getBytes());
        assertThat(originalRequest)
                .as("Non Http method request. Original request should not be modified.")
