...
class DirectForwardServerIT {
    ...
    @Tag("IntegrationTest")
    @DisplayName(
            "DFIT001: Assert that a Connection established is received when sending a connect request to the "
                    + "DirectForwardServer.")
    void assertForward1() throws ProxyEvaluationException, IOException {
        ...
        String connectRequest =
                "CONNECT remote.server.com:8080 HTTP/1.1\r\nHost: remote.server.com:8080\r\n";
        int directServerPort = directForwardServer.getPort();
        // create a testing client which connects to the DirectForwardServer and can send and
        // receive messsages
        ...
        assertThat(Util.CONNECTION_ESTABLISHED)
                .as("A Connection established should be received " + "but that is not the case.")
    ...
    void assertForward2() throws ProxyEvaluationException, IOException {
        ...
        assertThat(getRequest)
                ...
                .isEqualTo(responseFromTestingServer);
    }
}
