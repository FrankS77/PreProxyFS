 ...
 * The DirectForwardServer will handle all requests coming from the {@link DistributeForwardClientThread}
 * to serve direct connections (without proxies) to remote servers.
 *
 * @author Frank Schullerer
 */
public class DirectForwardServer extends Thread {

    private static final Logger LOGGER = LoggerFactory.getLogger(DirectForwardServer.class.getName());
     ...
     * Create forward thread to read responses from remote server (server socket) and forward
    ...
    public void run() {
        ...
        try (ServerSocket serverSocket = new ServerSocket(0)) {
            ...
            LOGGER.info("Start DirectForwardServer on TCP port : {}", this.serverSocketD.getLocalPort());
            synchronized (waitForMe) {
                waitForMe.notifyAll();
            }
            ...
            throw new PreProxyFSException("Unable to bind DirectForwardServer to local port. Program exit.", e);
        } catch (SocketException e) {
            if (e.getMessage().equals("Socket closed")) {
                LOGGER.info("Closing DistributeServer socket");
            } else {
                throw new PreProxyFSException("Error creating DirectForwardServer socket, Program exit.", e);
            }
        } catch (IOException e) {
            throw new PreProxyFSException("Error creating DirectForwardServer socket, Program exit.", e);
        }
