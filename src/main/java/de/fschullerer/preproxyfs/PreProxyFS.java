 ...
 *
 * <p>PreProxyFS is designed to solve the following problem: For example, one is in a corporate
 * network and needs to use a PAC script (https://en.wikipedia.org/wiki/Proxy_auto-config) to reach
 * certain Internet or intranet addresses but the program that needs this access can only use a
 * normal proxy on one port. In addition, to use one or more defined proxies, a username and
 * password must be entered (Basic Authentication). This is also not supported by several programs
 * (unlike Internet browsers).
 *
 ...
 * proxies defined in it and if necessary provided with the Basic Authentication header, so that no
 * user name and password input are necessary any more.
 *
...
public class PreProxyFS {
    ...
    /** The list contain remote proxies that were started by {@link #startProxyForwardServer}. */
    static final CopyOnWriteArrayList<ProxyForwardServer> proxyForwardServerList =
            new CopyOnWriteArrayList<>();
    /**
     * With {@link PacScriptParser#evaluate} can we check which proxy to use for a specific
     * Intranet/Internet address.
     */
     ...
     * @return The full Proxy-Authorization HTTP request header line. Ready to insert it to request
     *     header.
     */
    ...
    /**
     * Get the stored/defined proxy authentication (from settings file) for a special proxy.
     *
    ...
    private static void readSettings(String settingsFilePath) {
        ...
        try (FileInputStream settingsFile = new FileInputStream(settingsFilePath);
                InputStreamReader in =
                        new InputStreamReader(settingsFile, StandardCharsets.UTF_8)) {
            props.load(in);
        } catch (Exception e) {
            throw new PreProxyFSException(
                    "Failure loading the settings file: " + settingsFilePath + " Program exit.", e);
        }
    ...
    /**
     * Setting (optional): Get the proxy username password combinations from USER_PASSWORD_MAP
     * settings file and set {@link #proxyAuthenticationMap}.
     *
     * @param usersAndPassForProxies The USER_PASSWORD_MAP property value. Can be empty.
    ...
    static Map<String, String[]> setUserPasswordMap(String usersAndPassForProxies) {
        ...
        if (!"".equals(usersAndPassForProxies)) {
            ...
            if ((leftSquareBracketCount < 4) || leftSquareBracketCount != rightSquareBracketCount) {
                throw new PreProxyFSException(
                        "The configuration for USER_PASSWORD_MAP is wrong. Tip: Count the square"
                                + " brackets [ and ] in variable. It must be 4 for each entry. Program exit.");
            }
    ...
    private static void addProxyToList(String hostName, String proxyPort) {
        ProxyForwardServer remoteProxyForward =
                new ProxyForwardServer(hostName, Integer.parseInt(proxyPort));
        proxyForwardServerList.add(remoteProxyForward);
    }

    /**
     * Start a {@link DirectForwardServer} for every proxy in the PAC script. Every {@link
     * DirectForwardServer} will create a local socket to communicate with the proxy.
     *
    ...
    /**
     * Start PreProxyFS with this method, if you want to start PreProxyFS from within another JAVA
     * program.
     *
     * @param pacFilePathOrUrl Mandatory: URL or file path to the settings properties file. e.g.
     *     http://remote.proxy.pac.server.com/proxy.pac or file:/c:/myProxy.pac
     * @param localPortBind Optional (can be null, default value is than used). The local port to
     *     bind to. This local port and the following ports (localPortBind +1 +2 +3 ...) must not be
     *     in use! Better use a higher port. Default is 65000. If a lower port than 80 is set, the
     *     default value is used.
     * @param proxyAuth Optional (can be null). If one or more proxies in the defined PAC script use
     *     basic authentication, put a new key/value pair to this map e.g.
     *     the.first.proxy.in.pac.script.com:8080, new
     *     String[]{"usernameForProxy","passwordForProxy"}. Attention: If there is a square bracket
     *     [ or ] in your password, please use &#91; for left square bracket [ and use &#93; for
     *     right square bracket
     */
    public static void startPreProxyFS(
            String pacFilePathOrUrl, Integer localPortBind, Map<String, String[]> proxyAuth) {
        try {
            LOGGER.info("----------------------------------------");
            LOGGER.info("");
            ...
            throw new PreProxyFSException(
                    "A fatal error occurred during startup. Program exit.", e);
        }
    }
    ...
    public static void stopPreProxyFS() {
        try {
            ...
            if (null != directForwardServer && null != directForwardServer.getServerSocketD()) {
                LOGGER.info(
                        "Try to stop direct connection thread for port: {}",
                        directForwardServer.getPort());
                directForwardServer.getServerSocketD().close();
            }
            if (null != mainDistributionServer
                    && null != mainDistributionServer.getServerSocket()) {
    ...
    public static void main(String[] args) {
        if (args.length == 0) {
            LOGGER.error(
                    "PreProxyFS needs as parameter the full path to the PreProxyFS.properties file!");
            LOGGER.error(
                    "Call PreProxyFS script like e.g. (Linux) sh PreProxyFS /path/to/PreProxyFS.properties ");
