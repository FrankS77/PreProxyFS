 ...
 * server socket (e.g. port 65001), creates a new connection to a remote server (e.g.
 * google.com:443), starts a new {@link ForwardServerThread} that will process the response from
 * that that server and forward the original request to the remote server.
 *
...
public class DirectForwardClientThread extends Thread implements ForwardServerThreadInterface {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(DirectForwardClientThread.class.getName());
    ...
    /**
     * Bind this DirectForwardClientThread to a forwarding server thread. So it is possible to get
     * the server socket.
     *
    ...
    /**
     * Directly write the Connection established response to the client socket because the server
     * socket (a remote server) does not understand a CONNECT request if it is not a proxy. CONNECT
     * requests are only for proxy connections.
     *
    ...
    private void handleHttpConnect(String httpReq) throws IOException {
        if (httpReq.startsWith("CONNECT")) {
            byte[] connectionEstablished =
                    Util.CONNECTION_ESTABLISHED.getBytes(StandardCharsets.US_ASCII);
            clientSocket.getOutputStream().write(connectionEstablished);
        }
    }

    /**
     * Handle only the Http (non encrypted) requests. Also the CONNECT method: Even for Https
     * connections: the first connection from client e.g. browser is to this proxy (PreProxyFS) and
     * this is a Http proxy. The CONNECT method should show destincation remote host and port.
     *
     * @param httpReq The Http request as string.
    ...
    /**
     * Read from client socket and write to the server socket until it is possible. If reading or
     * writing can not be done (due to exception or when the stream is at his end) or writing is
     * failed, exits the thread.
     */
    @Override
    public void run() {
        byte[] request;
        try {
            while (true) {
                ...
                // communication should always start with CONNECT (proxy connection) and then GET,
                // POST ...
